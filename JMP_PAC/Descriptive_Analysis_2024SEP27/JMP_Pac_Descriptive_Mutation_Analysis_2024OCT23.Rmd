---
title: "Descriptive Analysis on Geneomic Alterations in 120 JMP_Pac Patients using BMI >= 30 as Obese patient cut-off"
...
# write.table(wts_sequenced, file = '/oceanus/collab/InternalJeff/users/yxp177/./../hxd052/data/JMP_PAC/clinical_data/jmp_pac_masld30_status_2024OCT23.txt',sep = '\t', quote = F)
```
...
nafld.hypertension <- wts_sequenced %>%
  tabyl(`NASH/NAFLD Status (Y/N)`,`Hypertension (Y/N)`) %>%
  gt(
...
nafld.hyperlipidemia <- wts_sequenced %>%
  tabyl(`NASH/NAFLD Status (Y/N)`,`Hyperlipidemia Status (Y/N)`) %>%
  gt(
    rowname_col = 'NASH/NAFLD Status (Y/N)'
  ) %>%
  tab_spanner(
...
nafld.dm <- wts_sequenced %>%
  tabyl(`NASH/NAFLD Status (Y/N)`,`DM`) %>%
  gt(
    rowname_col = 'NASH/NAFLD Status (Y/N)'
  ) %>%
  tab_spanner(
...
MASLD patients between BMI $\ge$ 25 or 30 is not significantly different.
...
kras.coding.gg <- ggplot(data = kras_mutation) + 
  geom_mosaic(aes(x = product(KRAS_Mutation, MASLD25), fill = KRAS_Mutation), divider = c('vbar','hspine')) +
  theme_minimal() +
  theme(axis.ticks.x = element_blank(), panel.grid.major = element_blank()) + 
  labs(y = 'KRAS mutation', x = 'MASLD Status', title = paste('KRAS nucleotide changes\nFisher test p-value = ',signif(fisher.test(with(kras_mutation, table(KRAS_Mutation, MASLD25)))$p.value, digits = 3), sep = ''))
kras.coding.gg
```

```{r KRAS protein changes plot, echo=FALSE}
kras.protein <- with(kras_mutation, table(KRAS_Protein,MASLD25))

kras.protein.gg <- ggplot(data = kras_mutation) + 
  geom_mosaic(aes(x = product(KRAS_Protein, MASLD25), fill = KRAS_Protein), divider = c('vbar','hspine')) +
  theme_minimal() +
  theme(axis.ticks.x = element_blank(), panel.grid.major = element_blank()) + 
  labs(y = 'KRAS AA Changes', x = 'MASLD Status', title = paste('KRAS amino acid changes\nFisher test p-value = ',signif(fisher.test(kras.protein)$p.value, digits = 3), sep = ''))
kras.protein.gg
```
...
kras.G12D <- with(kras_mutation, table(MASLD25,G12D))

kras.g12d.gg <- ggplot(data = kras_mutation) + 
  geom_mosaic(aes(x = product(G12D, MASLD25), fill = G12D), divider = c('vbar','hspine')) +
  theme_minimal() +
  theme(axis.ticks.x = element_blank(), panel.grid.major = element_blank()) + 
  labs(y = 'KRAS G12D v other changes', x = 'MASLD Status', title = paste('KRAS G12D status\nFisher test p-value = ',signif(fisher.test(kras.G12D)$p.value, digits = 3), sep = '')) +
...
```{r BRAF CNA-seq table, echo=FALSE}
  ...
  mut_table %>%
    ...
    tab_stubhead(
      label = 'MASLD30 Status'
    ) %>%
    tab_footnote(
      footnote = paste('Fisher test p-value = ',signif(fisher.test(mut_table$MASLD30,mut_table$result)$p.value, digits = 3))
      )
```
```{r BRAF DNA-seq table, echo=FALSE}
  ...
  # colnames(mut_table)[10] <- cancer_biomarker[1]
  #with(mut_table, table(result_group,MASLD30))
  mut_table %>%
    tabyl(`MASLD30`,`result`) %>%
    gt(
      rowname_col = 'MASLD30'
...
```{r Microsatellite Instability (MSI) table, echo=FALSE}
  ...
  mut_table %>%
    ...
    tab_footnote(
      footnote = paste('Fisher test p-value = ',signif(fisher.test(mut_table$MASLD30,mut_table$result_group)$p.value, digits = 3))
      )
```
