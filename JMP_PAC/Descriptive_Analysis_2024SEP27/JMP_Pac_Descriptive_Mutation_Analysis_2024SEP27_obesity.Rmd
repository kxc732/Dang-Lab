...
This report focused on Obesity status and genomics alterations. The criterion for obesity status is BMI $\ge$ 25.
...
```{r Obese25 v Hypertension, echo=FALSE}
...
```{r Obese25 v Hyperlipidemia, echo=FALSE}
...
kras.coding.gg <- ggplot(data = kras_mutation) + 
  ...
  labs(y = 'KRAS mutation', x = 'Obesity25 Status', title = paste('KRAS nucleotide changes\nFisher test p-value = ',signif(fisher.test(with(kras_mutation, table(KRAS_Mutation, Obese25)))$p.value, digits = 3), sep = ''))
kras.coding.gg
```
...
```{r BRAF CNA-seq table, echo=FALSE}
  ...
  mut_table %>%
    ...
    tab_footnote(
      footnote = paste('Fisher test p-value = ',signif(fisher.test(mut_table$Obese25,mut_table$result)$p.value, digits = 3))
      )
```
