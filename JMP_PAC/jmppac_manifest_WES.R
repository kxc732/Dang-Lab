...
# Date: 09/10/2024
# Description: Script searches data repository and creates a manifest for WES DNA files.
...
bam_files <- list.files(path = "WES/BAM", pattern = "^DNA.*\\.bam$", full.names = file_mode)
fastq_r1_files <- list.files(path = "WES/FASTQ", pattern = "^DNA.*.R1.*\\.fastq.gz$", full.names = file_mode)
fastq_r2_files <- list.files(path = "WES/FASTQ", pattern = "^DNA.*.R2.*\\.fastq.gz$", full.names = file_mode)
vcf_files <- list.files(path = "WES/vcf", pattern = "^DNA.*\\.vcf$", full.names = file_mode)
pdf_files <- list.files(path = "WES/reports/pdf", pattern = "^TN.*\\.pdf$", full.names = file_mode)
xml_files <- list.files(path = "WES/reports/xml", pattern = "^TN.*\\.xml$", full.names = file_mode)
json_files <- list.files(path = "WES/reports/json", pattern = "^TN.*\\.json$", full.names = file_mode)
...
for (bam_file in bam_files) {
    ...
    file_info <- strsplit(basename(bam_file), "_|\\.")[[1]]
    CARIS_ID <- file_info[2]
    df <- rbind(df, data.frame(JMPPAC_ID = "", CARIS_ID = CARIS_ID, WES_DNA_BAM = bam_filename, 
                               WES_DNA_FASTQ_R1 = "", WES_DNA_FASTQ_R2 = "", WES_DNA_VCF = "", 
                               WES_REPORT_PDF = "", WES_REPORT_XML = "", stringsAsFactors = FALSE))
}

for (fastq_file in fastq_r1_files) {
    fastq_filename <- ifelse(full_path, fastq_file, basename(fastq_file))
    file_info <- strsplit(basename(fastq_file), "_|\\.")[[1]]
    CARIS_ID <- file_info[2]
...
for (vcf_file in vcf_files) {
    vcf_filename <- ifelse(full_path, vcf_file, basename(vcf_file))
    file_info <- strsplit(basename(vcf_filename), "_|\\.")[[1]]
    CARIS_ID <- file_info[2]
    df$WES_DNA_VCF[df$CARIS_ID == CARIS_ID] <- vcf_filename
}

for (pdf_file in pdf_files) {
    pdf_filename <- ifelse(full_path, pdf_file, basename(pdf_file))
...
for (xml_file in xml_files) {
    xml_filename <- ifelse(full_path, xml_file, basename(xml_file))
...
for (json_file in json_files) {
    json_filename <- ifelse(full_path, json_file, basename(json_file))
...
# write.csv(df, "/oceanus/collab/InternalJeff/users/kxc732/Projects/../../hxd052/data/JMP_PAC/datasets/manifests/WES_manifest.csv")
